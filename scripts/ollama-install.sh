...
configure_runit() {
    ...
    $SUDO mkdir -p /etc/runit/sv/ollama/log
    $SUDO mkdir -p /var/log/ollama

    # Create run script with environment setup
    cat <<EOF | $SUDO tee /etc/runit/sv/ollama/run >/dev/null
#!/bin/sh
exec 2>&1
...
EOF
    $SUDO chmod +x /etc/runit/sv/ollama/run

    # Create log service
    cat <<EOF | $SUDO tee /etc/runit/sv/ollama/log/run >/dev/null
#!/bin/sh
exec svlogd -tt /var/log/ollama
EOF
    $SUDO chmod +x /etc/runit/sv/ollama/log/run
    ...
    for SVDIR in /run/runit/service /var/service /service; do
        if [ -d "$SVDIR" ]; then
            ...
            $SUDO ln -sf /etc/runit/sv/ollama $SVDIR/
            break
        fi
    done
